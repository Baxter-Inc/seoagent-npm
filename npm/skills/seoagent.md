...
18. **Respect the session budget** (see "Session Economy — bounded sessions"). Target under ~60 turns and under ~20 new files per session; consolidate writes, batch bookkeeping into one final update and batch multi-file code edits, scope single-session asks to audit + plan + top fixes, and reserve the final turns for `seoagent verify-recs` + `seoagent summary` (see "Ending a Session") — no new workstreams after the summary, and never start one you can't finish.
